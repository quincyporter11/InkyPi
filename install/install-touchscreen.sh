...
# =============================================================================
# Script Name: install-touchscreen.sh
# Description: This script Installs InkyPI for use as a Raspberry Pi
#              touchscreen dashboard.
#
# Usage: sudo ./install-touchscreen.sh
#        
# This installer configures:
#        - InkyPi
#        - Chromium
#        - Touchscreen-native input
#        - Dashboard kiosk mode
#
# E-ink display hardware configuration is intentionally skipped.
# =============================================================================
...
install_touchscreen_dependencies() {
    echo "Installing touchscreen dashboard dependencies."
...
configure_touchscreen() {
    ...
    if [[ ! -f "$LABWC_CONFIG" ]]; then
        echo_error "labwc configuration not found at $LABWC_CONFIG"
        return
    fi

    cp "$LABWC_CONFIG" \
       "${LABWC_CONFIG}.inkypi-backup"

    sed -i \
        '/generic ft5x06/s/mouseEmulation="yes"/mouseEmulation="no"/g' \
        "$LABWC_CONFIG"

    echo_success "\tNative FT5x06 touchscreen input enabled."
}

configure_touchscreen_device() {
    local DEVICE_JSON="$SRC_PATH/config/device.json"

    echo "Configuring InkyPi for touchscreen dashboard mode."
...
config["display_type"] = "mock"
config["resolution"] = [800, 480]
...
PY
    ...
    echo_success "\tTouchscreen display configuration applied."
}

# Dashboard Launcher For Touchscreen
install_dashboard_launcher() {
    local LAUNCHER_PATH="/usr/local/bin/inkypi-dashboard"

    echo "Installing touchscreen dashboard launcher."
...
# Wait for InkyPi to become available before starting Chromium.
...
EOF
    ...
    echo_success "\tDashboard launcher installed at $LAUNCHER_PATH"
}

# Configure Dashboard Auto Start
configure_dashboard_autostart() {
    ...
    if [[ -z "$INSTALL_HOME" ]]; then
        echo_error "Unable to determine home directory for $INSTALL_USER"
        exit 1
    fi
    ...
    echo "Configuring dashboard kiosk autostart for $INSTALL_USER."

    mkdir -p "$LABWC_DIR"

    # Preserve an existing autostart file.
    if [[ -f "$AUTOSTART_FILE" ]]; then
        if ! grep -q '/usr/local/bin/inkypi-dashboard' "$AUTOSTART_FILE"; then
            echo "" >> "$AUTOSTART_FILE"
            echo "/usr/local/bin/inkypi-dashboard &" >> "$AUTOSTART_FILE"
        fi
    else
        cat > "$AUTOSTART_FILE" <<'EOF'
#!/bin/sh

/usr/local/bin/inkypi-dashboard &
EOF
    fi
    ...
    echo_success "\tDashboard kiosk autostart configured."
}
