...
class Weather(BasePlugin):
    ...
    def get_render_context(self, settings, device_config):
        ...
        time_format = device_config.get_config(
            "time_format",
            default="12h"
        )
        ...
        try:
            if weather_provider == "OpenWeatherMap":
                api_key = device_config.load_env_key(
                    "OPEN_WEATHER_MAP_SECRET"
                )

                if not api_key:
                    raise RuntimeError(
                        "Open Weather Map API Key not configured."
                    )
                ...
                if settings.get(
                    ...
                    logger.info(
                        "Using location timezone for OpenWeatherMap data."
                    )

                    render_tz = self.parse_timezone(weather_data)

                else:
                    logger.info(
                        "Using configured timezone for OpenWeatherMap data."
                    )
                ...
                raise RuntimeError(
                    f"Unknown weather provider: {weather_provider}"
                )
            ...
            logger.error(
                f"{weather_provider} request failed: {str(e)}"
            )
        ...
        if time_format == "24h":
            last_refresh_time = now.strftime(
                "%Y-%m-%d %H:%M"
            )
        else:
            last_refresh_time = now.strftime(
    ...
    def generate_image(self, settings, device_config):
        template_params = self.get_render_context(
            settings,
            device_config
        )
        ...
        if not image:
            raise RuntimeError(
                "Failed to take screenshot, please check logs."
            )
